# 📑 Documentation Index - Blackhorse Publishing v2.0

**Your complete guide to the Blackhorse Publishing website.**

---

## 🚀 Start Here

### New to this project?
1. 📖 **[QUICKSTART.md](QUICKSTART.md)** ← Deploy in 5 minutes
2. 📖 **[README.md](README.md)** ← Project overview
3. 📖 **[DEPLOYMENT.md](DEPLOYMENT.md)** ← Full deployment guide

### Just want to deploy?
👉 **[QUICKSTART.md](QUICKSTART.md)** - Upload 7 files, done!

---

## 📚 All Documentation

### Essential Guides
| File | Purpose | Read Time |
|------|---------|-----------|
| **[QUICKSTART.md](QUICKSTART.md)** | Deploy in 5 minutes | 2 min |
| **[README.md](README.md)** | Project overview & features | 5 min |
| **[DEPLOYMENT.md](DEPLOYMENT.md)** | Platform-specific deployment | 10 min |

### Technical Reference
| File | Purpose | Read Time |
|------|---------|-----------|
| **[FILES.md](FILES.md)** | File manifest & dependencies | 5 min |
| **[MANIFEST.md](MANIFEST.md)** | Complete file inventory | 5 min |
| **[VERSION.md](VERSION.md)** | Version history | 3 min |

### Release Information
| File | Purpose | Read Time |
|------|---------|-----------|
| **[RELEASE-NOTES-v2.0.md](RELEASE-NOTES-v2.0.md)** | What's new in v2.0 | 8 min |
| **[ATTRIBUTIONS.md](ATTRIBUTIONS.md)** | Credits & licenses | 2 min |

---

## 🎯 Quick Reference by Task

### "I want to deploy the site"
→ **[QUICKSTART.md](QUICKSTART.md)** - 5-minute guide  
→ **[DEPLOYMENT.md](DEPLOYMENT.md)** - Detailed platform guides

### "I want to understand the file structure"
→ **[FILES.md](FILES.md)** - File dependencies  
→ **[MANIFEST.md](MANIFEST.md)** - Complete inventory

### "I want to see what changed"
→ **[VERSION.md](VERSION.md)** - Version history  
→ **[RELEASE-NOTES-v2.0.md](RELEASE-NOTES-v2.0.md)** - v2.0 changes

### "I want to know what files to upload"
→ **[QUICKSTART.md](QUICKSTART.md)** - Lists the 7 required files  
→ **[FILES.md](FILES.md)** - Full breakdown

### "I want the complete overview"
→ **[README.md](README.md)** - Start here

---

## 📦 Production Files (Upload These)

**Only 7 files needed:**

1. ✅ `index.html` - Homepage
2. ✅ `defense.html` - Defense Systems page
3. ✅ `publishing.html` - Publishing page
4. ✅ `white-paper.html` - White Paper catalog
5. ✅ `institutional-white-paper.html` - Institutional page
6. ✅ `styles.css` - Global stylesheet
7. ✅ `script.js` - Navigation functionality

**Total size:** ~48 KB  
**Dependencies:** None (except Google Fonts CDN)

---

## 🌐 Live Site Structure

After deployment, your site will have these URLs:

```
https://blackhorsepublishing.net/
https://blackhorsepublishing.net/defense.html
https://blackhorsepublishing.net/publishing.html
https://blackhorsepublishing.net/white-paper.html
https://blackhorsepublishing.net/institutional-white-paper.html
```

---

## 📧 Contact & Support

**General inquiries:** contact@blackhorsepublishing.net  
**Technical support:** contact@blackhorsepublishing.net

---

## 🔄 Version Information

**Current Version:** 2.0 Official  
**Release Date:** February 19, 2026  
**Architecture:** Multi-page static HTML  
**Build Step:** None  
**Dependencies:** None

---

## ✅ Quick Checklist

Before deploying:

- [ ] Read **QUICKSTART.md** (5 minutes)
- [ ] Collect the 7 required files
- [ ] Choose hosting platform (Cloudflare/Netlify recommended)
- [ ] Upload files to root directory
- [ ] Test navigation and email links
- [ ] Connect custom domain
- [ ] Done! 🎉

---

## 📊 Documentation Statistics

- **Total documentation files:** 8
- **Total word count:** ~8,000 words
- **Total read time:** ~40 minutes
- **Quick deploy time:** 5 minutes

---

## 🏆 Key Features

✅ Zero build dependencies  
✅ 5 fully-functional pages  
✅ Interactive navigation dropdown  
✅ Email integration with smart subject lines  
✅ Responsive design  
✅ SEO-friendly structure  
✅ ~48 KB total size  
✅ Works on any static host  

---

## 📖 Reading Order (Recommended)

**For new users:**
1. INDEX.md (this file) - Overview
2. QUICKSTART.md - Deploy quickly
3. README.md - Full understanding

**For deployers:**
1. QUICKSTART.md - Quick deploy
2. DEPLOYMENT.md - Platform specifics
3. FILES.md - Verify structure

**For developers:**
1. README.md - Project overview
2. MANIFEST.md - Complete inventory
3. FILES.md - Dependencies
4. RELEASE-NOTES-v2.0.md - Technical changes

---

## 🎯 Next Steps

1. ✅ Read **[QUICKSTART.md](QUICKSTART.md)**
2. ✅ Upload 7 files to your hosting
3. ✅ Test the live site
4. ✅ Connect custom domain
5. ✅ Share with the world!

---

**Welcome to Blackhorse Publishing v2.0 Official! 🎉**

*Last updated: February 19, 2026*
